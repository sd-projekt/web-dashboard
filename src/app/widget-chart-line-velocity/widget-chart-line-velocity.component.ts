...
export class WidgetChartLineVelocityComponent {
  ...
  constructor(private http: HttpClient) {
      ...
      this.getData().subscribe(data => {
        ...
        this.chartOptions= {
          series: [
            { type: 'line', xKey: 'time', yKey: 'velocity', yName: 'Velocity'},
          ],
          data: dataArray,
          background: {
            fill: "transparent",
  ...
  getData() {
    const velocityURL = "http://localhost:8000/data/velocity/value";

    return this.http.get<any>(velocityURL + "?fromWhen=1m").pipe(
      map((response) => {
        const resultArray = [];
        ...
        for (let i = 0; i < dataArray.length; i++) {
          const newDataEntry = {
            ...
            velocity: Number(dataArray[i].value)
          };
